...
def ClassWgtConstructFeasibleSolutionandHyperParams(**kwargs):
    ...
    for C in Cs:
        ...
        for i, (train_index, test_index) in enumerate(kf.split(period_Context)):
            ...
            # solve SVM
            try:
                svm_phase1_results = sklearn_ClassWgtSVM(period_Context.iloc[train_index], K, z_vals[train_index], C, separable)
            except ValueError:
                if Verbose:
            ...
            period_Context_subset = period_Context.iloc[:, q_largest]
            try:
                svm_phase2_results = sklearn_ClassWgtSVM(period_Context_subset.iloc[train_index], K, z_vals[train_index], C,
                                                 separable)
                svc = svm_phase2_results['svc']
                pred_decision = svc.decision_function(period_Context_subset.iloc[test_index])
            except ValueError:
                svm_phase2_results = ClassWgtSVM(period_Context_subset.iloc[train_index], K, z_vals[train_index], C, separable,
                                         bigM_limit_time, LogToConsole)
    ...
    try:
        ...
        svm_phase2_results = ClassWgtSVM(period_Context_subset, K, z_vals, bestC, separable, bigM_limit_time, LogToConsole)
    ...
    BestSubsetSVM_results = BestSubsetClassWgtSVM(period_Context, K, z_vals, bestC, separable, q, big_w2, bigM_limit_time,
